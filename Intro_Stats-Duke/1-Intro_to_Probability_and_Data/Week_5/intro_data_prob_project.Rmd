...
bad_days_by_state <- bad_days_by_state %>%
  mutate(region = as.character(tolower(state)))

bad_state <-inner_join(bad_days_by_state, states, by = "region")

ggplot(data = bad_state) + 
  geom_polygon(aes(x = long, y = lat, fill = avg_dev, group = group), color = "white") + 
  coord_fixed(1.3) +
