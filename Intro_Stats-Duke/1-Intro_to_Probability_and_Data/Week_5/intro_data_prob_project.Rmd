...
dep_cmp +
  geom_bar(aes(fill = dd), position = "fill") +
  geom_hline(yintercept = 0.5, alpha = 0.7, color = "red")
...
$ P(Depressed|\textup{More than 1 bad day} ) $


#TODO: Bayesian probability of being depressed given 30 bad days.
...
# geo <- geocode(location = as.character(bad_days_by_state$state), output = "latlon")

#bad_days_by_state$lat <- geo$lat
#bad_days_by_state$lon <- geo$lon

range_avg_dev <- range(bad_days_by_state$avg_dev)
range_arange_med_dev <- range(bad_days_by_state$med_dev)

usa <- map_data("usa")

#ggplot(data = states) + 
#  geom_polygon(aes(x = long, y = lat, fill = bad_days_by_state$avg_dev, group = group), color = "white") + 
#  coord_fixed(1.3) +
#  guides(fill=FALSE)
  

#pmh_map <- ggplot() +
#  geom_polygon(data = bad_days_by_state %>% na.omit(),
#               aes(x = lon, y = lat, fill = avg_dev)) +
#  coord_map()
